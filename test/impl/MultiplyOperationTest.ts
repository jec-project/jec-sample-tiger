...
@TestSuite({
  description: "Tests the methods of the MultiplyOperation class"
})
export class MultiplyOperationTest {
  ...
  public testCompute():void {
    ...
    const operation:Operation = new MultiplyOperation();
  ...
  public testIdempotency():void {
    ...
    const b:number = 4;
    const operation:Operation = new MultiplyOperation();
    expect(operation.compute(a, b)).to.equal(32);
  }
}